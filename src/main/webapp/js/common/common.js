...
//JJH 2022 03 10 팝업 화면 check
function goUrlFullDisplayCheck(field_url,targetName){
	//팝업 호출
	winArr =window.open(field_url,targetName,'height=' + screen.height + ',width=' + screen.width + 'fullscreen=yes');
	return winArr;
}
...
var regCheckPassword =/^(?=.*[A-Za-z])(?=.*\d)(?=.*[$@$!%*#?&-_|`~])[A-Za-z\d$@$!%*#?&-_|`~]{8,}$/
//사용 예시
/*
...
//< , > , & , "  변환
function replaceSpecial_type1(str){
...
function getCurrentDate(day){
	...
	if(day==""){
		...
		var date11 = new Date();
		var date = new Date(date11.setDate(date11.getDate()+ Number(day.substring(1,2))));
		toDay =date.getFullYear()+"-" + ("0"+(date.getMonth()+1)).slice(-2)+"-" + ("0"+date.getDate()).slice(-2);
		//console.log("++++++ : " + toDay);
	}else if (day.indexOf('-') != -1) {
		var date11 = new Date();
...
/**
 * 달력에서 사용할 날짜 2자리 , 3달전 날짜 구하는 함수    - 일경우만 사용가능
 * 
 * */

function getBeforeThreeMonthCurrentDate(){
	
	var toDay="";
	
	var date11 = new Date();
	var date = new Date(date11.setDate(date11.getDate() - 90 ));
	toDay =date.getFullYear()+"-" + ("0"+(date.getMonth()+1)).slice(-2)+"-" + ("0"+date.getDate()).slice(-2);
	return toDay;
}
...
////			console.log("분류 선택");
////			console.log(data);
//			listClassL = data.listClassL;
//			listClassM = data.listClassM;
//			listClassS = data.listClassS;
//			setClsL();
//		},
...
function setClsL(){
//	console.log("대분류 세팅");
	let selectClsL = document.getElementById("clsL");
	selectClsL.options.length = 0;
	selectClsL.add(new Option('선택', '0'));
	let selectClsM = document.getElementById("clsM");
	selectClsM.options.length = 0;
	selectClsM.add(new Option('선택', '0'));
	let selectClsS = document.getElementById("clsS");
	selectClsS.options.length = 0;
	selectClsS.add(new Option('선택', '0'));
	for (const cl of listClassL){
		selectClsL.add(new Option(cl.cls_name, cl.cls_seqno));
	}
}
...
//	console.log("중분류 세팅");
	...
	selectClsS.options.length = 0;
	selectClsS.add(new Option('선택', '0'));
	for (const cl of listClassM){
		if(cl.up_cls_seqno == selectClsL.value){
			selectClsM.add(new Option(cl.cls_name, cl.cls_seqno));
		}
	}
}
...
//	console.log("소분류 세팅");
	...
	selectClsS.options.length = 0;
	selectClsS.add(new Option('선택', '0'));
	for (const cl of listClassS){
		if(cl.up_cls_seqno == selectClsM.value){
			selectClsS.add(new Option(cl.cls_name, cl.cls_seqno));
		}
	}
}
...
function setCycle(){
	return new Promise((resolve) => {
		setTimeout(() => {
		//	console.log("수집주기 세팅");
			let selectCycleEvery = document.getElementById("cycleEvery").value;
			let selectCycleOn = document.getElementById("cycleOn");
			selectCycleOn.options.length = 0;
			selectCycleOn.add(new Option('선택', '0'));
			
			if(selectCycleEvery == 'day'){
				for(let i=1; i<=24; i++){
					selectCycleOn.add(new Option(i, i));
				}
			}else if(selectCycleEvery == 'time'){
				for(let i=1; i<=60; i++){
					selectCycleOn.add(new Option(i, i));
				}
			}else if(selectCycleEvery == 'weekday'){
				const weekday = new Map([['Monday', 0], ['Tuesday', 1], ['Wednesday', 2], ['Thursday', 3], ['Friday', 4], ['Saturdayday', 5], ['Sunday', 6]]);
				for(const [key, value] of weekday){
					selectCycleOn.add(new Option(key, value));
				}
			}else if(selectCycleEvery == 'specificday'){
				for(let i=1; i<=31; i++){
					selectCycleOn.add(new Option(i, i));
				}
			}
